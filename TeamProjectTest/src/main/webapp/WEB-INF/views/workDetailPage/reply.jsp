      ...
      <div class="review" id="11">
        ...
        <c:forEach var="CommentDTO" items="${list}">
         <div class="reply-box">      
          ...
          <input type="hidden" value="${CommentDTO.user_no }" name="user_no">
            <div class="user-icon">
	...
	$(".replyremoveBtn").on("click", function() {
	 ...
	 <c:if test="${not empty msg}">
		...
		$('.dropdown-item').on('click', function(event) {
			  ...
			  var target_user_no = $(this).closest('.reply-box').find('input[name="user_no"]').val(); // target_user_no 값을 가져옵니다.
			  var cmt_no = $(this).closest('.reply-box').find('input[name="cmt_no"]').val(); // cmt_no 값을 가져옵니다.
